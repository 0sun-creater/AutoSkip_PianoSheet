...
img_gray = cv2.imread('Images/doncry1.png',0)
img_rgb = cv2.imread('Images/doncry1.png')
...
#Write image
#cv2.imwrite("treble_staff2.jpg", img_point)
    ...
    if keypoints[i].pt[1] > first and keypoints[i].pt[1] < last:
        ...
        if k%2==0:
            ...
            for n in range(0,len(left),1):
                if n == len(left)-1:
                    break
                if abs(left[n+1][0] - left[n][0]) <7 and abs(left[n+1][1] - left[n][1]) < 6:
                    pop_arr2.append(n)
                elif n!=0 and abs(left[n+1][0] - left[n-1][0]) < 7 and abs(left[n+1][1] - left[n-1][1]) < 6:
                    pop_arr2.append(n-1)               
...
for n in range(0,len(left),1):
    if n == len(left)-1:
        break
    if abs(left[n+1][0] - left[n][0]) <5 and abs(left[n+1][1] - left[n][1]) < 5:
        pop_arr2.append(n)
    elif n!=0 and abs(left[n+1][0] - left[n-1][0]) < 5 and abs(left[n+1][1] - left[n-1][1]) < 5:
        pop_arr2.append(n-1)
...
for u in range(0,len(melody_),1):
    for z, p in dic_mel.items():
        if melody_[u] == z and flag == 0:
            cnt = cnt+1
            if cnt ==50 or cnt==54 or cnt == 120 or cnt ==124:
